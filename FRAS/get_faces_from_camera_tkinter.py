...
class Face_Register:
    def __init__(self):
        ...
        ipcam='http://172.25.169.249:8080/video'
        self.cap = cv2.VideoCapture(ipcam)  # Get video stream from camera
    ...
    def GUI_info(self):
        tk.Label(self.frame_right_info,
                 text="Face Register(Self Supervised Learning)",
                 font=self.font_title).grid(row=0, column=0, columnspan=3, sticky=tk.W, padx=3, pady=18)
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="Step 1: Clear face photos").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        tk.Button(self.frame_right_info,
                  text='Clear',
                  command=self.GUI_clear_data).grid(row=10, column=0, columnspan=3, sticky=tk.W, padx=5, pady=2)

        
        # Step 2: Input name and create folders for face
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 2: Input name").grid(row=12, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        tk.Label(self.frame_right_info, text="Name: ").grid(row=13, column=0, sticky=tk.W, padx=5, pady=0)
        self.input_name.grid(row=13, column=1, sticky=tk.W, padx=0, pady=2)

        tk.Button(self.frame_right_info,
                  text='Input',
                  command=self.GUI_get_input_name).grid(row=13, column=2, padx=5)

        # Step 3: Save current face in frame
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 3: Save face image").grid(row=15, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        # Show log in GUI
        self.log_all.grid(row=19, column=0, columnspan=20, sticky=tk.W, padx=5, pady=20)
